(ns pubchem.main
  "Kotodama WASM entrypoint for the Pubchem clean-room actor (L4) — Clojure port.
  ...
  facts (`pubchem.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pubchem")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "pubchem_wor"
    :fields [:doi :title :type :publishedAt]  :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"    :plural "authors"    :id-prefix "pubchem_aut"
    :fields [:orcid :givenName :familyName]   :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "pubchem_cit"
    :fields [:workId :citedDoi]                :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "pubchem_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "pubchem_rec"
    :fields [:accession :database :title]     :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "pubchem_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"pubchem.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pubchem-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pubchem-compat/ready)
